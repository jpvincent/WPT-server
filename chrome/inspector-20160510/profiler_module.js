...
{return this._linkifier;},_ensureFlameChartCreated:function()
{if(this._flameChart)
return;this._dataProvider=new WebInspector.CPUFlameChartDataProvider(this.profile,this._profileHeader.target());this._flameChart=new WebInspector.CPUProfileFlameChart(this._searchableView,this._dataProvider);this._flameChart.addEventListener(WebInspector.FlameChart.Events.EntrySelected,this._onEntrySelected.bind(this));},_onEntrySelected:function(event)
...
break;var urlElement=this.tree._formatter.linkifyNode(this);urlElement.style.maxWidth="75%";cell.appendChild(urlElement);break;default:cell=WebInspector.DataGridNode.prototype.createCell.call(this,columnId);break;}
...
{this.tree=this;this.children=[];this._formatter=formatter;this._searchableView=searchableView;this.total=total;this.lastComparator=null;this.childrenByCallUID={};}
...
profileNode.UID=++profileNodeUIDs;if(profileNode.parent){var visitedNodes=visitedProfileNodesForCallUID[profileNode.callUID];var totalAccountedFor=false;if(!visitedNodes){visitedNodes={};visitedProfileNodesForCallUID[profileNode.callUID]=visitedNodes;}else{var parentCount=parentProfileNodes.length;for(var parentIndex=0;parentIndex<parentCount;++parentIndex){if(visitedNodes[parentProfileNodes[parentIndex].UID]){totalAccountedFor=true;break;}}}
visitedNodes[profileNode.UID]=true;this._remainingNodeInfos.push({ancestor:profileNode,focusNode:profileNode,totalAccountedFor:totalAccountedFor});}
...
{return this._cpuProfile.profileHead.total;},maxStackDepth:function()
...
this._maxStackDepth=maxDepth;this._timelineData=new WebInspector.FlameChart.TimelineData(entryLevels,entryTotalTimes,entryStartTimes,null);this._entryNodes=entryNodes;this._entrySelfTimes=entrySelfTimes;return this._timelineData;},_millisecondsToString:function(ms)
{if(ms===0)
return"0";if(ms<1000)
...
{return true;},supportsRegexSearch:function()
{return false;},__proto__:WebInspector.VBox.prototype};WebInspector.CPUProfileFlameChart.OverviewCalculator=function()
{}
...
WebInspector.CPUProfileView.prototype={columnHeader:function(columnId)
{switch(columnId){case"self":return WebInspector.UIString("Self Time");case"total":return WebInspector.UIString("Total Time");}
return"";},__proto__:WebInspector.ProfileView.prototype}
...
{WebInspector.ProfileNode.call(this,node.functionName,node.scriptId,node.url,node.lineNumber,node.columnNumber);this.self=node.selfSize;this.callUID=`${this.frame.functionName}@${this.frame.scriptId}:${this.frame.lineNumber}`;}
...
{var callFrame=node.profileNode.frame;return this._profileView.linkifier().linkifyConsoleCallFrame(this._profileView.target(),callFrame,"profile-node-file");}};WebInspector.HeapSnapshotProgressEvent={Update:"ProgressUpdate",BrokenSnapshot:"BrokenSnapshot"};WebInspector.HeapSnapshotCommon={}
